...
public class APITestReportUtils {
    ...
    public static String getPrintableReport(APIBaseResponse response, String testMethodName, String reportName) {
        StringBuilder htmlLog = new StringBuilder();


        System.out.println("Generating Printable Report for test :" + response.getTestName());
        htmlLog.append(getSummary(response.getDescription(), response.getTestType(), response.getTotalRequests(), response.getTotalFailed()));
        htmlLog.append(START_TEST_FOLDER);
        if (APIDataSourceType.excel.toString().equals(response.getReportFormat())) {
            htmlLog.append(generateEXCEL(response, testMethodName, reportName));
        } else {
            htmlLog.append(generateHTML(response));
        }

        htmlLog.append(endDiv());
        htmlLog.append(getMismatchFrequencyHtml(APITestUtils.getMismatchedFieldFrequencyMap(response)));
        htmlLog.append("<br/>");
    ...
    protected static String getResponseCodeVerificationLog(APIScenarioResponse responseItem) {
        String message = null;
        boolean result = true;
    ...
    public static String generateHTML(APIBaseResponse apiResponse) {
    ...
    public static String generateEXCEL(APIBaseResponse apiResponse, String testName, String filePath) {
        if (apiResponse.getTestError() != null) {
            return "<b><FONT COLOR=\"RED\">API Test Error : <br/>" + ExceptionUtils.getStackTrace(apiResponse.getTestError()) + "</FONT></b><br/>";
        }
        if (apiResponse.getTotalRequests() <= 0) {
        ...
        if (apiResponse instanceof APIResponse)
            return new APIExcelReporter().report((APIResponse) apiResponse, testName, filePath);
    ...
    public static String emptyResponseMessage(String testName) {
